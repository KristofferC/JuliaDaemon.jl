...
set -uo pipefail
if [ -n "${JLD_E2E_TRACE:-}" ]; then set -x; JLD_ERR=/dev/stderr; else JLD_ERR=/dev/null; fi
...
out=$($J eval 'using ToyPkg; ToyPkg.greet()' 2>"$JLD_ERR")
check "autostart+load" 0 $?
checkout "greet output" "Hello World!" "$out"

out=$($J eval '1 + 1' 2>"$JLD_ERR")
...
EOF
out=$($J eval 'ToyPkg.double(21)' 2>"$JLD_ERR")
...
out=$($J run "$WORK/scratch.jl" 2>"$JLD_ERR"; $J eval 'scratch_result' 2>"$JLD_ERR")
checkout "run file + state persists" "55" "$out"

out=$($J --module=Wk eval 'wk_x = 7; wk_x' 2>"$JLD_ERR"; $J eval 'Main.Wk.wk_x * 2, isdefined(Main, :wk_x)' 2>"$JLD_ERR")
checkout "module workspace" "(14, false)" "$out"

out=$($J eval 'sc_ref = 5;' 2>"$JLD_ERR"; $J eval-scratch 'sc_tmp = sc_ref * 3' 2>"$JLD_ERR"; $J eval 'isdefined(Main, :sc_tmp)' 2>"$JLD_ERR")
checkout "eval-scratch sees Main, keeps nothing" "15
false" "$out"

# ---- introspection ----

out=$($J stacks 2>"$JLD_ERR")
check "stacks" 0 $?
checkout "stacks reports threads" "Thread" "$out"

out=$($J eval 'Main.JLDDaemon.id()' 2>"$JLD_ERR")
checkout "daemon knows its id" "ToyPkg-$NAME" "$out"

# ---- targeting ----

out=$($JLD --id=ToyPkg-$NAME eval '"by id"' 2>"$JLD_ERR")
checkout "eval by --id prefix" "by id" "$out"

out=$($JLD --project="$WORK/ToyPkg" --name=iso eval 'iso_var = 1; "iso ok"' 2>"$JLD_ERR")
checkout "second named daemon" "iso ok" "$out"
out=$($J eval 'isdefined(Main, :iso_var)' 2>"$JLD_ERR")
checkout "named daemons are isolated" "false" "$out"

out=$($JLD --project="$WORK/ToyPkg" list 2>"$JLD_ERR")
checkout "list shows both daemons" "ToyPkg-iso" "$out"

# ---- transcript ----

$J eval 'for i in 1:5000; println("spam ", i); end; "spammed"' >/dev/null 2>&1
out=$($J transcript 2>"$JLD_ERR")
...
out=$($JLD --id=ToyPkg-ci eval 'sess_var + 1' 2>"$JLD_ERR")
...
out=$($JLD --id=ToyPkg-ci eval '"session alive"' 2>"$JLD_ERR")
...
out=$($J eval '"still here"' 2>"$JLD_ERR")
...
if [ $FAILS -eq 0 ]; then
    ...
    for d in "$XDG_CACHE_HOME"/julia-daemon/*/; do
        ...
        tail -30 "$d/daemon.log" 2>"$JLD_ERR"
    done
    exit 1
fi
